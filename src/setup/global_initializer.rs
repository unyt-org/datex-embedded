...
use core::fmt::Debug;
use datex_core::{network::com_hub::ComHub, runtime::Runtime};
...
impl Debug for CommonContext {
    fn fmt(&self, f: &mut core::fmt::Formatter<'_>) -> core::fmt::Result {
        f.debug_struct("CommonContext").finish()
    }
}
...
impl GlobalInitializer {
    ...
    pub async fn init_datex_runtime(
        ...
        let (current_time_us, maybe_wifi_stack) = match wifi_credentials {
            Some(wifi_credentials) => {
                #[cfg(feature = "wifi")]
                {
                    let wifi_stack = wifi_initializer
                        .init_wifi_stack(&spawner, wifi_credentials)
                        .await;
                    let timestamp_generator =
                        setup_initializer.get_timestamp_generator();
                    (
                ...
                {
                    panic!(
                        "Cannot initialize DATEX runtime with WIFI, 'wifi' feature is disabled"
                    )
                }
            }
            None => (0, None),
    ...
    async fn init_network_stack(
        ...
        get_network_time(stack, timestamp_generator).await.unwrap()
    }
}
