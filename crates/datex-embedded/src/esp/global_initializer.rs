use crate::{
    esp::timestamp_generator::TimestampGenerator,
    setup::global_initializer::{
        SetupInitializer, WifiCredentials, WifiInitializer,
    },
};
use alloc::rc::Rc;
...
impl SetupInitializer for EspSetupInitializer {
    fn register_com_interface_factories(
        &self,
        ...
        {
            use crate::interfaces::websocket_client_interface_embedded::{
                WebSocketClientInterfaceEmbeddedGlobalState,
                WebSocketClientInterfaceSetupDataEmbedded,
            };
            if let Some(stack) = stack {
                use esp_hal::rng::Rng;

                WebSocketClientInterfaceEmbeddedGlobalState::set_global_state(
                    WebSocketClientInterfaceEmbeddedGlobalState {
                        stack: stack.clone(),
                        rng: Rc::new(Rng::new()),
                    },
        ...
        {
            use crate::interfaces::tcp_client_interface_embedded::{
                TCPClientInterfaceSetupDataEmbedded,
                TcpClientInterfaceEmbeddedGlobalState,
            };
            if let Some(stack) = stack {
                use esp_hal::rng::Rng;

                TcpClientInterfaceEmbeddedGlobalState::set_global_state(
                    TcpClientInterfaceEmbeddedGlobalState {
                        stack: *stack,
                        rng: Rc::new(Rng::new()),
                    },
